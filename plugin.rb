...
# about: Shop plugin (ready build) with admin product creation + image upload, public listing and debug endpoints
# version: 1.11.1-ready
...
after_initialize do
  ...
  begin
    ...
    require_dependency File.expand_path('../app/controllers/discourse_shop_pro/admin/products_controller.rb', __FILE__)
    require_dependency File.expand_path('../app/controllers/discourse_shop_pro/debug_controller.rb', __FILE__)
  rescue => e
    ::DiscourseShopPro::DEBUG[:boot_errors] << "require controllers: #{e.class}: #{e.message}\n#{e.backtrace&.first(5)&.join("\n")}"
  end
...
Discourse::Application.routes.append do
  ...
  get  '/shop/admin/products'  => 'discourse_shop_pro/admin/products#index'
  post '/shop/admin/products'  => 'discourse_shop_pro/admin/products#create'
end
